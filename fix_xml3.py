...
def fix_file(file_path):
    ...
    for i in range(1, 5):
        # Find start of block
        match = re.search(f'<LinearLayout\s+android:id="@+id/slotImage{i}".*?android:gravity="center_horizontal"(.*?)>', content, re.DOTALL)
        if not match:
            print(f"Skipping {i} in {file_path}, no start tag")
            continue
        ...
        # Check if already a FrameLayout outer (already fixed)
        if '<FrameLayout\n                    android:id="@+id/slotImage' in block or '<FrameLayout\n                android:id="@+id/slotImage' in block:
            print(f"Skipping {i} in {file_path}, seems fixed")
            continue
            
        # extract marginEnd
        margin_end_match = re.search(r'android:layout_marginEnd="(\d+dp)"', match.group(0))
